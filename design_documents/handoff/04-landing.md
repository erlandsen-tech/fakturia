  ...
  return (
    <div className="bg-paper-grain text-ink">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[1.15fr_1fr] gap-12 px-6 lg:px-12 pt-16 pb-24 max-w-7xl mx-auto items-center">
        <div>
          ...
          <h1 className="font-display text-[64px] md:text-[96px] leading-[0.98] tracking-[-0.025em] m-0">
            {t("landing.heroLineA")} <em>{t("landing.heroEm1")}</em>,<br />
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 border-b border-ink/10 max-w-7xl mx-auto">
        {[
          { I: StampMoth,   t: t("landing.value1Title"), b: t("landing.value1Body") },
          { I: InkwellTide, t: t("landing.value2Title"), b: t("landing.value2Body"), highlight: true },
          { I: CoinStones,  t: t("landing.value3Title"), b: t("landing.value3Body") },
        ].map(({ I, t: title, b, highlight }, i) => (
          <div
            key={i}
            className={[
              "p-9",
          ...
          >
            ...
            <p className="text-sm text-ink-2 leading-[1.6] mt-2.5 m-0">{b}</p>
          </div>
        ))}
      </section>
...
›   • mark_paid`}
        ...
        <div className="flex justify-center"><StampMoth size={400} /></div>
      </section>
      ...
      <section className="px-6 lg:px-12 py-24 border-t border-ink/10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            { name: t("pricing.tier1"), price: "9",   unit: t("pricing.unit1"), note: t("pricing.note1"), btn: t("pricing.btn1") },
            { name: t("pricing.tier2"), price: "199", unit: t("pricing.unit2"), note: t("pricing.note2"), btn: t("pricing.btn2"), best: true },
            { name: t("pricing.tier3"), price: "399", unit: t("pricing.unit3"), note: t("pricing.note3"), btn: t("pricing.btn3") },
          ].map((p) => (
            <div
              key={p.name}
              className={[
            ...
            >
              {p.best && (
                <div className="cap absolute top-5 right-5 text-sun">{t("pricing.best")}</div>
              )}
              <div className="font-display text-[28px]">{p.name}</div>
              <div className="flex items-baseline gap-2 mt-4">
                <span className="font-display text-[72px] leading-none tracking-[-0.02em]">{p.price}</span>
                <span className={p.best ? "text-sm text-ink-mute" : "text-sm text-ink-3"}>{p.unit}</span>
              </div>
              <p className={["text-sm mt-3.5 leading-[1.5]", p.best ? "text-paper-2/70" : "text-ink-2"].join(" ")}>
                {p.note}
              </p>
              <Button
...
The current root `<Navbar/>` shouldn't appear on the marketing page (it's an in-app chrome). Hide it on `/`:

```tsx
// in src/components/navbar.tsx, at the top of the component:
...
Strings move to `src/lib/i18n.ts` (chapter 09). Defaults shown above are Norwegian (the primary locale).

## Commit

```
landing: rewrite page.tsx to brand-led layout (paper hero, manifesto, MCP block, three-tier pricing)
```

Continue → [05-public-invoice.md](./05-public-invoice.md)
